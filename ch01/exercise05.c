...
**
** This program uses a shuffler to reorder the column numbers (excepting the
**  final negative number). It then rearranges as it originally should.
*/
 ...
 *  columns. The output line is then NULL terminated.
 */
void rearrange(char *output, char const *input, int n_columns, int columns[]) {
    ...
    for (col = 0; col < n_columns - 1; col++) {
        if (col + 1 < 0)    break;
        for (counter = col + 1; counter < n_columns; counter++) {
            if (columns[col] > columns[counter]) {
                temp = columns[col];
                columns[col] = columns[counter];
                columns[counter] = temp;
            }
        }
    }
    ...
    for (col = 0; col < n_columns; col += 2) {
        ...
        // If the input line isn't this long or the output array is full,
        // we're done.
